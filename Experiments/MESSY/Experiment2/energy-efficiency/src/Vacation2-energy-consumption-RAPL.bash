...
# -s the number of  worker actors. (this is only available for txact version) 
		...
		do
			...
			output_name="output${j}-$1-$2-vacation2-linux.txt"
		       ...
		       # output=$(sudo perf stat -o perf.txt -r 1 -e power/energy-cores/ lein run -- -v $1 -w ${i} -s $2 -t ${num_reservations} -r ${num_flights_cars} -n ${num_queries} -p ${p} && ${output_name} 1> perf.data)
		        ...
		        output=$(sudo perf stat -o perf-${j}-${i}-$1.txt -r 1 -e power/energy-cores/,power/energy-ram/,power/energy-gpu/,power/energy-pkg/ lein run -- -v $1 -w ${i} -s $2 -t ${num_reservations} -r ${num_flights_cars} -n ${num_queries} -p ${p} )			
